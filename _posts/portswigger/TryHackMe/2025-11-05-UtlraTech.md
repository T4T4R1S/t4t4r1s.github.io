...
## Web Enumeration 
**Access node js server** I find page that's have UltraTech API v0.1.3 

![](/assets/TryHackMeRoomsImage/UltraTech/1.png)

**Access apache server** : Normal website and nothing interesting

![](/assets/TryHackMeRoomsImage/UltraTech/2.png)

## Brute Force using ffuf
**brute force apache server to show hidden dirctory**
...
```
 ...
 `/js` contain `api.js` i find this source code :
![](/assets/TryHackMeRoomsImage/UltraTech/3.png)
This code execute code from nodejs server to get hostname it's command injection:
...
![](/assets/TryHackMeRoomsImage/UltraTech/4.png)
 ...
 it say www let's try another one 

 **ls**
![](/assets/TryHackMeRoomsImage/UltraTech/5.png)

 list file call `utech.db.sqlite`  database file trying to cat it :

![](/assets/TryHackMeRoomsImage/UltraTech/6.png)

it's contain 2 user first one called r00t and the second called admin and there  password hashes
...
we are `r00t` not root but we are in docker group that's all we need
