---
layout: post
title: HTML SOURCE CODE
subtitle: HTML SOURCE CODE
description: HTML SOURCE CODE
image: https://www.root-me.org/IMG/logo/siteon0.svg?1637496509
optimized_image: https://www.root-me.org/IMG/logo/siteon0.svg?1637496509
category: [Root Me, web server]
tags:
  - RootME
  - HTML - web TECH
  - Find Password
author: mustafa_altayeb
date: 2026-02-02 00:00:00 +0000
paginate: true
---

# HTML Source code - RootMe

![alt text](/assets/Rootmeimages/image2.png)

1) view page source .

2) in last comment i found the password and take challenge points .

 ![alt text](/assets/Rootmeimages/image3.png)

Finished. Happy Hacking!



<iframe src="https://tryhackme.com/api/v2/badges/public-profile?userPublicId=3186403" style="border:none;"></iframe>

Follow me:
- [LinkedIn](https://www.linkedin.com/in/t4t4r1s/)
- [X](https://x.com/T4T4R1S)

---
